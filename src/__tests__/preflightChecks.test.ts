...
describe('preflightChecks', () => {
  ...
  describe('checkNodeVersion', () => {
    it('should throw NodeVersionError for Node < 18', () => {
  ...
  describe('checkPackageAvailability', () => {
    it('should throw PackageFetchError when fetch fails', async () => {
  ...
  describe('validateFoundryToken', () => {
    it('should return cached token when still valid', async () => {
      ...
      expect(await validateFoundryToken(foundryApiUrl, CLI_TOKEN)).toBe(CACHED_TOKEN)
    })
    ...
    it('should return refreshed token when cached token is expired', async () => {
      ...
      expect(await validateFoundryToken(foundryApiUrl, CLI_TOKEN)).toBe(REFRESHED_TOKEN)
    })
    ...
    it('should fall back to CLI token when cached token is invalid', async () => {
      ...
      expect(await validateFoundryToken(foundryApiUrl, CLI_TOKEN)).toBe(CLI_TOKEN)
    })
    ...
    it('should use git token for browser auth bootstrap', async () => {
      vi.mocked(gitConfigParser.parseGitToken).mockReturnValue('git-token')
      mockRefreshTokenIfExpired.mockResolvedValue(REFRESHED_TOKEN)

      expect(await validateFoundryToken(foundryApiUrl, undefined)).toBe(REFRESHED_TOKEN)
    })

    it('should not return git token directly even if TTL is valid', async () => {
      vi.mocked(gitConfigParser.parseGitToken).mockReturnValue('git-token')
      mockRefreshTokenIfExpired.mockResolvedValue(undefined)
